...
val df = spark.read.format("csv").option("header", "true").option("inferSchema", "true").option("mode", "DROPMALFORMED").csv("b3.csv")
...
val data = pipedata.select('status,'exercisetype,'foodfeatures)
...
val data1 = pipedata1.select('status,'exercisefeatures,'foodfeatures)




val assembler = new VectorAssembler().setInputCols(Array("exercisefeatures", "foodfeatures")).setOutputCol("features")
